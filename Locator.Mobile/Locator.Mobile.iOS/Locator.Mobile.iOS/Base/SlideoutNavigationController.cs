...
namespace Locator.Mobile.iOS.Base
{
    ...
    public class SlideoutNavigationController : UIViewController
    {
        ...
        private bool _rightMenuEnabled;
        ...
        {
            get { return _internalTopView.View.BackgroundColor; }
            set { _internalTopView.View.BackgroundColor = value; }
        }
        ...
        private void Pan(UIView view)
        {
            if (_panGesture.State == UIGestureRecognizerState.Began)
            {
                ...
                if (!Visible)
                {
                    var touch = new PointF();
                    try
                    {
                        touch = _panGesture.LocationOfTouch(0, view);
                    }
                    catch
                    {
                    }
                      ...
                      _panGesture.State == UIGestureRecognizerState.Cancelled))
            {
                ...
                if (Visible)
                {
                    if ((view.Frame.X < (view.Frame.Width/2) && _leftMenuShowing) ||
                        (view.Frame.X > -(view.Frame.Width/2) && _rightMenuShowing))
                        Hide();
                    else if (_leftMenuShowing)
                    {
                        UIView.Animate(SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
                            () =>
                            {
                                view.Frame = new RectangleF(SlideWidth, view.Frame.Y, view.Frame.Width,
                                    view.Frame.Height);
                            }, () => { });
                    }
        ...
        public override void ViewDidLoad()
        {
            ...
            _internalMenuViewRight.View.Frame = new RectangleF(View.Frame.Width - SlideWidth, 0, SlideWidth,
                View.Frame.Height);
        ...
        public void ShowMenuLeft()
        {
            ...
            if (_internalTopView.View.Subviews != null && _internalTopView.View.Subviews.Length > 0)
            {
                _internalTopView.View.Subviews[0].UserInteractionEnabled = false;
            }

			if (_internalMenuViewLeft.View.Subviews.Length > 0)
			{
				_internalMenuViewLeft.View.Subviews[0].UserInteractionEnabled = true;
			}
            ...
            UIView.Animate(SlideSpeed, 0, UIViewAnimationOptions.CurveEaseInOut,
                () =>
                {
                    _internalTopView.View.Frame = new RectangleF(SlideWidth, 0, _internalTopView.View.Frame.Width,
                        _internalTopView.View.Frame.Height);
                }, () =>
                {
                    if (_internalTopView.View != null)
                    {
                        _internalTopView.View.AddGestureRecognizer(_tapGesture);
                    }
                });
        }
        ...
        public void SelectView(UIViewController view)
        {
            if (_internalMenuViewLeft.View.Subviews.Length > 0)
            {
                _internalMenuViewLeft.View.Subviews[0].UserInteractionEnabled = false;
            }
            ...
            _internalTopNavigation = new UINavigationController(view)
            {
                View =
                {
                    Frame = new RectangleF(0, 0, _internalTopView.View.Frame.Width, _internalTopView.View.Frame.Height)
                }
            };
        ...
        public void Hide(bool animate = true)
        {
            ...
            NSAction animation =
                () =>
                {
                    _internalTopView.View.Frame = new RectangleF(0, 0, _internalTopView.View.Frame.Width,
                        _internalTopView.View.Frame.Height);
                };
            NSAction finished = () =>
            {
                if (_internalTopView.View.Subviews.Length > 0)
                {
                    _internalTopView.View.Subviews[0].UserInteractionEnabled = true;
                }
                _internalTopView.View.RemoveGestureRecognizer(_tapGesture);
        ...
        private class SlideoutPanDelegate : UIGestureRecognizerDelegate
        {
            ...
            public override bool ShouldReceiveTouch(UIGestureRecognizer recognizer, UITouch touch)
            {
                return (_controller.Visible ||
                       ...
                       (_controller.LeftMenuEnabled || _controller.RightMenuEnabled);
            }
        }
